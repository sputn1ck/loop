...
func TestSweepBatcherBatchCreation(t *testing.T) {
	...
	batcher := NewBatcher(lnd.WalletKit, lnd.ChainNotifier, lnd.Signer,
		testMuSig2SignSweep, nil, lnd.ChainParams, store)
	go func() {
		err := batcher.Run(ctx)
	...
	store.AssertSweepStored(sweepReq1.SwapHash)
	store.AssertSweepStored(sweepReq2.SwapHash)
	store.AssertSweepStored(sweepReq3.SwapHash)
}
...
func TestSweepBatcherSimpleLifecycle(t *testing.T) {
	...
	// this time instead of monitoring the spend it starts monitoring the
...
func TestSweepBatcherSweepReentry(t *testing.T) {
	...
	// Since the first batch progressed its state to "closed" it should have
	// registered a monitor for confirmations.
	<-lnd.RegisterConfChannel
...
func TestSweepBatcherComposite(t *testing.T) {
	...
	store.AssertSweepStored(sweepReq2.SwapHash)
	store.AssertSweepStored(sweepReq3.SwapHash)
	store.AssertSweepStored(sweepReq4.SwapHash)
	store.AssertSweepStored(sweepReq5.SwapHash)
	store.AssertSweepStored(sweepReq6.SwapHash)
}
