...
func (m *MigratorManager) migrateLoopOuts(ctx context.Context) error {
	...
	swapContracts := make([]*LoopOutContract, len(loopOuts))
	swapHashes := make([]lntypes.Hash, len(swapContracts))
	updateHashes := []lntypes.Hash{}
	updatesTimes := []time.Time{}
	updateStates := []SwapStateData{}

	// For each loop out, create a new loop out in the toStore.
	for i, loopOut := range loopOuts {
		swapContracts[i] = loopOut.Contract
		swapHashes[i] = loopOut.Hash

		for _, event := range loopOut.Events {
			updateHashes = append(updateHashes, loopOut.Hash)
			updatesTimes = append(updatesTimes, event.Time)
			updateStates = append(updateStates, event.SwapStateData)
		}
	}

	// Create the loop outs in the toStore.
	err = m.toStore.BatchCreateLoopOut(ctx, swapHashes, swapContracts)
	if err != nil {
		return err
	}
	...
	err = m.toStore.BatchInsertUpdate(
		ctx, updateHashes, updatesTimes, updateStates,
	)
	if err != nil {
		return err
	}
...
func (m *MigratorManager) migrateLoopIns(ctx context.Context) error {
	...
	swapContracts := make([]*LoopInContract, len(loopIns))
	swapHashes := make([]lntypes.Hash, len(swapContracts))
	updateHashes := []lntypes.Hash{}
	updatesTimes := []time.Time{}
	updateStates := []SwapStateData{}
	...
	// For each loop out, create a new loop out in the toStore.
	for i, loopIn := range loopIns {
		swapContracts[i] = loopIn.Contract
		swapHashes[i] = loopIn.Hash

		for _, event := range loopIn.Events {
			updateHashes = append(updateHashes, loopIn.Hash)
			updatesTimes = append(updatesTimes, event.Time)
			updateStates = append(updateStates, event.SwapStateData)
		}
	}
	...
	// Create the loop outs in the toStore.
	err = m.toStore.BatchCreateLoopIn(ctx, swapHashes, swapContracts)
	if err != nil {
		return err
	}
