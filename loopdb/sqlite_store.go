...
func (s *BaseDB) FetchLoopOutSwaps(ctx context.Context) ([]*LoopOut,
	...
	err := s.ExecTx(ctx, NewSqlReadOpts(), func(*sqlc.Queries) error {
		...
		for _, swap := range swaps {
			...
			loopOuts = append(loopOuts, loopOut)
		}
...
func (s *BaseDB) CreateLoopOut(ctx context.Context, hash lntypes.Hash,
	...
	return s.ExecTx(ctx, writeOpts, func(tx *sqlc.Queries) error {
		...
		htlcKeyInsertArgs := swapToHtlcKeysInsertArgs(hash, swap.SwapContract)
...
func (s *BaseDB) BatchCreateLoopOut(ctx context.Context, hashes []lntypes.Hash,
	swaps []*LoopOutContract) error {

	writeOpts := &SqliteTxOptions{}
	return s.ExecTx(ctx, writeOpts, func(tx *sqlc.Queries) error {

		if len(hashes) != len(swaps) {
			return errors.New("hashes and swaps must be of equal length")
		}
		...
		for i, swap := range swaps {
			...
			loopOutInsertArgs := loopOutToInsertArgs(hashes[i], swap)
...
func (s *BaseDB) FetchLoopInSwaps(ctx context.Context) (
	...
	err := s.ExecTx(ctx, NewSqlReadOpts(), func(*sqlc.Queries) error {
		...
		for _, swap := range swaps {
			...
			loopOut, err := s.convertLoopInRow(
				swap, updates,
			)
			if err != nil {
				return err
			...
			loopIns = append(loopIns, loopOut)
		}
...
// BatchCreateLoopOut adds multiple initiated swaps to the store.
func (s *BaseDB) BatchCreateLoopIn(ctx context.Context, hashes []lntypes.Hash,
	...
	return s.ExecTx(ctx, writeOpts, func(tx *sqlc.Queries) error {
		...
		for i, swap := range swaps {
			...
			loopInInsertArgs := loopInToInsertArgs(hashes[i], swap)
...
func (s *BaseDB) BatchInsertUpdate(ctx context.Context, hashes []lntypes.Hash,
	times []time.Time, states []SwapStateData) error {

	writeOpts := &SqliteTxOptions{}
	return s.ExecTx(ctx, writeOpts, func(tx *sqlc.Queries) error {

		if len(hashes) != len(times) || len(hashes) != len(states) {
			return errors.New("hashes, times and states must be of equal length")
		}
		...
		for i, hash := range hashes {
			...
			if states[i].HtlcTxHash != nil {
				updateParams.HtlcTxhash = states[i].HtlcTxHash.String()
			}
			// First we insert the swap update.
			err := tx.InsertSwapUpdate(ctx, updateParams)
			if err != nil {
...
func loopInToInsertArgs(hash lntypes.Hash,
	...
	loopInInsertParams := sqlc.InsertLoopInParams{
		SwapHash:       hash[:],
		HtlcConfTarget: int32(loopIn.HtlcConfTarget),
	...
	*LoopOut, error) {
	...
	loopOut := &LoopOut{
		Contract: &LoopOutContract{
			...
			SweepConfTarget:         int32(row.SweepConfTarget),
...
func (s *BaseDB) convertLoopInRow(row sqlc.GetLoopInSwapsRow,
	...
	loopIn := &LoopIn{
		Contract: &LoopInContract{
			...
			HtlcConfTarget: int32(row.HtlcConfTarget),
