...
func (f *FSM) DepositStatesV0() fsm.States {
	return fsm.States{
		...
		PublishExpirySweep: fsm.State{
			...
			Action: f.PublishExpirySweepAction,
		},
...
func isUpdateSkipped(notification fsm.Notification,
	...
	// If a deposit failed for some reason, we don't update it. Instead, we
	// keep it ready for another action. Say a loop-in failed, we can still
	// try to withdraw the deposit.
	if prevState == Deposited && checkStateFunc(Failed) {
		return true
	}
