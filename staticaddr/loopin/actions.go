...
func (f *FSM) MonitorInvoiceAndHtlcTxAction(ctx context.Context,
	...
	timeout := time.Duration(f.loopIn.PaymentTimeoutSeconds) * time.Second
	serverPaymentDeadline := time.NewTimer(timeout)
	...
	for {
		...
		case currentHeight := <-blockChan:
			...
			// htlcConfChan, we wrongfully reset the deposits in the
			// code below. Hence, we delay the timeout check here by
			// one block to ensure that htlcConfChan fires first.
...
func (f *FSM) MonitorHtlcTimeoutSweepAction(ctx context.Context,
	...
	for {
		...
		case conf := <-htlcTimeoutTxidChan:
			...
			f.Infof("htlc timeout sweep tx confirmed at block-"+
				"height %v", conf.BlockHeight)
